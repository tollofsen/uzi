...
/alias gg /gg
/alias gl glance %{*}
/alias glog bleedlog show group%; heallog show group%;fraglog show group%;hitlog show group%;explog show group%;damlog show group
/alias glogr bleedlog reset group%; heallog reset group%;hitlog reset group%;explog reset group%;damlog reset group
/alias gr group %{*}
...
/alias weap /w %{*}
/alias pick /if (leading=1 & rogue<1) gtell pick %{*}%;/else /send pick %{*}%;/endif
...
/else \
	/alias peek /send peek %%{*}%%;/set _peek_peekdir %%{*}%;\
/endif
...
/alias weara hold %{heldeq}%;wear all
...
/if (animist > 0) \
	...
	/if (animist=2) \
		...
		/alias swarm cast 'Deadly Swarm' swarm%;\
		/alias cphoenix cast 'Conjure Phoenix'%;\
		/alias nb cast 'Natures Blessing'%;\
		/alias ser cast 'Serenity'%;\
		/alias serenity cast 'Serenity'%;\
	/endif%;\
/endif
