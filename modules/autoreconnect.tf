...
/def -mglob -p2 -F -t'\[REBOOT\] Rebooting. Burning will be back up in about 60 seconds!' reboot_handler = \
		/set pgmode=0%;\
		/uzi_stablog_reset

/def -msimple -F -t'[NO FIGHTING] Rebooting in 5 SECONDS...' pre_reboot_handler = \
		/set pgmode=0%;\
		/uzi_stablog_reset
;  /if (autoreconnect=1) \
