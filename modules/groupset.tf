...
/def -p1 -F -msimple -t'You stand up.' restspell6 = \
    /if (standtocast != 1) \
        /set position=stand%;\
        /aftertick%;\
        /onstand%;\
...
;;;;HEALTRIGGERS ETC....
    /def -p1 -F -mregexp -t'^[0-9]+\. \[([ 0-9]+)\]* (..|  | Mob)(\/..|   | )\] ([A-Za-z]+|[A-Za-z]+\'s (Vampire|Spectre|Ghast|Wolf|Phoenix|Earth Elemental|Air Elemental|Water Elemental)|Ceng, the friend of Eowaran|A giant sea turtle|A Seagull|.* named \'([A-Za-z]+)\')[ ]*\[(...)\%H (...)\%M ...\%V\] ([A-z0-9\.\%]+)' gpr3 = \
    /if (_aheal_mod=~'') \
    ...
    /let combo=%{P2}%{P3}%;\
    /if (combo=~'Pr/Pr') \
        /setCurrentPriest %{P4} %{P1} %{P8}%;\
    /endif%;\
    /if (combo=~'An/An') \
        /setCurrentAnimist %{P4} %{P1} %{P8}%;\
    /endif%;\
; Tankheal
    /if (aheal=1 & sentgroup=2 & {P9} !/ 'NotHere*' & {P9} & (priest>0 | templar>1 | animist>1)) \
...
;            /endif%;\
        /elseif (lowesthps <= atgmhp & miragroup=1 & templar=2) \
            /if (wildmagic=0) \
                cast 'miracle' %{toheal}%;\
		/set lspell=%;\
            /endif%;\
...
/def purgeGroupSettings = \
    /set currentPriest=%;\
    /set currentPriestMana=0%;\
    /set currentAnimist=%;\
    /set currentAnimistMana=0

/def setCurrentPriest = \
    /let _mana=$[strip_attr({3})]%;\
    /if ({2}>51 & _mana>currentPriestMana) \
       /set currentPriest=%{1}%;\
       /set currentPriestMana=%{_mana}%;\
    /endif

/def setCurrentAnimist = \
    /let _mana=$[strip_attr({3})]%;\
    /if ({2}>51 & {3}>currentAnimistMana) \
       /set currentAnimist=%{1}%;\
       /set currentAnimistMana=%{_mana}%;\
    /endif
