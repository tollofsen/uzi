...
/def checkwimp = \
    ...
    /if ((currenthp <= miraselfhp) & (selfmira=1) & (miraingself!=1) & (priest>1|templar>1)) \
        /if (currentmana>0) \
            mira self%;\
            /ecko Trying to Miracle self! %{currenthp} is lower than %{miraselfhp}!%;\
            /set miraingself=1%;/repeat -0:00:10 1 /set miraingself=0%;\
        /else \
            /ecko You got negative mana and can't mira self!!!!%;\
            ps%;\
            /set miraingself=1%;\
            /repeat -0:00:10 1 /set miraingself=0%;\
        /endif%;\
    /endif%;\
    /if (autowimpy=1 & fighting=1 & pgmode<1) \
