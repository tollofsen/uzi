...
/def -aBCred -mglob -t'You lost your concentration!' recast = \
	/if (sanc=0 & autoholy=1 & priest>0 & fighting=1) \
		cast 'Holyword'%;\
		/set sentdamage=0%;\
	/elseif (nightblade>0 & fighting=1 & lspell=~"cast \'Adrenal Focus\'") \
		adr%;\
	/elseif (fighting>0) \
		/repeatdamage%;\
...
/def -aBCred -msimple -F -p233312 -t'You cant seem to do that here!' nomag = \
	/set lspell=nothing%;/set spellup=null%;/set exitspellup=0%;/set give_leader=0%;/set nomag=1%;\
...
/def -Fp1 -aCmagenta -mglob -t'The last drop of venom drips off of *.' lostvenom = \
	/respell venom

/def -F -p2 -aCmagenta -msimple -t'You return to your original form.' lostshadowform

/def -F -p2 -aCmagenta -msimple -t'You feel less peaceful.' lostserenity =\
	/respell serenity

/def -Fp2 -aCmagenta -msimple -t'You seem a little less faithful.' lostgodsblessing = \
	/respell godsblessing
...
/def -p2 -aBCmagenta -mglob -t'Your shadow surrounds you, refusing to leave your presence.' gotsd2 = /set sd=1%;/gotspell sd

/def -p2 -aBCmagenta -mregexp -t'^[A-z]+\'s shadow refused to merge with your shadow.' gotsd3 = /set sd=1%;/gotspell sd
...
/def -p2 -aBCmagenta -F -msimple -t'The shadow realm engulfs your body. You stop breathing.' gotshadowform = \
	/set shadowform=1

/def -msimple -p2 -aBCmagenta -t'You gather upon the calming forces of nature to add vigor to your comrades!' gotserenity1 = \
	/set serenity=1%;/gotspell serenity

/def -msimple -p2 -aBCmagenta -t'You are already experiencing all that nature has to offer.' gotserenity2 = \
	/set serenity=1%;/gotspell serenity

/def -mregexp -Fp2 -aBCmagenta -t'^You feel a boost of vitality as .* channels the power of nature into your body.$' gotserenity3 = \
	/set serenity=1%;/gotspell serenity

/def -mregexp -aBCmagenta -Fp2 -t'^You feel refreshed as your god answers your plea.$|^You feel refreshed as your god answers your plea.$' gotgodsblessing = \
	/set godsblessing=1%;/gotspell godsblessing
...
/def -F -p100 -mglob -t'Blindness               {\[*|P*}*' afblind=/set blind=1
/def -F -p100 -mglob -t'Critical Wounds         {\[*|P*}*' afcurecritic=/set curecritic=1
/def -F -p100 -mglob -t'Shadow form             {\[*|P*}*' afshadowform=/set shadowform=1
/def -F -p100 -mglob -t'Serenity                {\[*|P*}*' afserenity=/set serenity=1
/def -F -p100 -mglob -t'Gods Blessing           {\[*|P*}*' afgodsblessing=/set godsblessing=1
...
/def dospellup = \
	/if (playing=1 & position=~'stand') \
		/set lspell= %;/set spellingup=1%;\
		/if (sanc=0 & autoholy=1 & priest>0 & ingroup>0 & level>31) \
			cast 'Holyword'%;\
			/set holy=0%;\
			/set spellup=holy%;\
		/elseif ((sanc=0 & ((templar>0 & level>=40)|(priest>0 & level >=13))) & (ingroup=0 | solo=1)) \
;        /elseif (sanc=0 & autoholy=1 & wildmagic=0 & ((templar>0 & level>=40)|(priest>0 & level>=13)) & solo=1) \
	cast 'Sanctuary'%;\
	/set holy=0%;\
	/set spellup=sanc%;\
/elseif ((blinded>0|withered>0) & animist>1 & currentmana>100 & level>33) \
	cast 'tranquility'%;\
	/set spellup=tranq%;\
/elseif (blind=1 & (priest|anmist)>0) \
	cast 'Cure Blind'%;\
	/set spellup=cb%;\
/elseif (blind=1 & templar>0) \
	cast 'Heal'%;\
	/set spellup=cb%;\
...
/elseif (regen=0 & priest>0 & level>=14) \
	cast 'regenerate'%;\
	/set spellup=regen%;\
/elseif (haste=0 & (warlock|magician)>0 & level>=25 & spellcaster<1) \
	cast 'haste'%;\
	/set spellup=haste%;\
/elseif (combat=0 & warlock>0 & level>=20 & spellcaster<1) \
	cast 'Combat'%; \
	/set spellup=combat%;\
/elseif (templar>1 & godsblessing=0 & level>62) \
	cast 'Gods Blessing'%;\
	/set spellup=godsblessing%;\
...
/elseif (mh=0 & nightblade>0 & level>=20) \
	cast 'Mountain Heart'%; \
	/set spellup=mh%;\
/elseif (str=0 & ((magician>0 & level>=4)|(templar>0 & level>=8)|(warlock>0 & level>=7)) & (strspell=1|level<=50|solo=1)) \
	cast 'Strength'%;\
	/set spellup=str%;\
/elseif (dv=0 & ((magician>0 & level>=13)|(nightblade>1 & level>=15))) \
	cast 'Darkvision'%; \
	/set spellup=dv%;\
/elseif (gsize=0 & animist>0 & level>=25 & spellcaster<1 & (solo>0|strspell=1)) \
	cast 'Giant Size'%;\
	/set spellup=gsize%;\
...
/elseif (ms=0 & ritual=0 & amshield=1 & warlock>0 & level>=35 & manalevel=~'high') \
	cast 'Mana Shield'%; \
	/set spellup=ms%;\
/elseif (ingroup=1 & ms=0 & ritual=0 & aritual=1 & level>=53 & warlock>0) \
	cast 'Ritual Of Pain'%; \
	/set spellup=ritual%;\
...
/elseif (fly=0 & magician>0 & level>=15) \
	cast 'fly'%;\
	/set spellup=fly%;\
/elseif (lblood=0 & nightblade>1 & level>=30 & solo=1) \
	cast 'life blood'%;\
	/set spellup=lblood%;\
...
/elseif (animist>1 & level>58 & serenity=0 & ingroup=1 & solo<1 & leading<1) \
	cast 'serenity'%;\
	/set spellup=serenity%;\
/elseif (curecritic=1 & (priest|templar|animist)>0) \
	cast 'cure critic' self%;\
	/set spellup=curecritic%;\
/elseif (regen=0 & (warlock|magician|animist|templar)>0 & wildmag<1) \
